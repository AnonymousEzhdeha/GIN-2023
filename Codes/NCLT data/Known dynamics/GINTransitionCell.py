...
class GINTransitionCell(k.layers.Layer):
    ...
    def __init__(self,
        ...
        :param init_Q_matrices: Q matrix initialization
        :param init_KF_matrices: Kalman matrix initialization
        :param Qnetwork: determines the type of network for infering the process noise.
    ...
    def build(self, input_shape):
        ...
        F = np.array([[1., delta_t,  0., 0.],
                         ...
                         [0., 0.,  1., delta_t],
                         [0., 0.,  0., 1.]])
        self.F_matrix = tf.convert_to_tensor(np.tile( np.expand_dims(np.array(F).astype(np.float32),0) ,
                                [input_shape[0], 1, 1]))
    ...
    def call(self, inputs, states, **kwargs):
        ...
        if self.Initialize_States <2:
            state_mean = tf.convert_to_tensor([[obs_mean[i,0].numpy(), 0., obs_mean[i,1].numpy(), 0.] for i in range(len(obs_mean))])
            #state_mean = tf.expand_dims(state_mean, axis=0)
            self.Initialize_States += 1
        ...
        self.save_post_state = post_state
    ...
    def _predict(self, post_mean, post_covar):#inputs are mu_t-1|t-1 and sigma_t-1|t-1 at time t
        ...
        self.transition_matrix = self.F_matrix
