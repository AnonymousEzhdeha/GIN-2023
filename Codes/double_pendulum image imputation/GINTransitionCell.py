...
class DynamicsNet:
    ...
    def __init__(self, lsd, number_of_basis, hidden_units):
        ...
        for u in hidden_units:
            layer = k.layers.Dense(u, activation=k.activations.relu, name ='DynamicNet {}th layer'.format(u))
            layer.build([None, cur_in_shape])
            cur_in_shape = u
            self._hidden_layers.append(layer)
        self._out_layer = k.layers.Dense(number_of_basis, activation=k.activations.softmax, name ='DynamicNet last layer')
    ...
    def build(self, input_shape):
        ...
        if self.onelayervar:
            ...
            Init_Hmatrix = np.tile( np.expand_dims(np.array(self.init_kf_matrices * np.random.randn(self._lod, self._lsd).astype(np.float32)),0) ,
                                    [1, self._num_basis, 1, 1])
            self.Hmatrix = tf.Variable(Init_Hmatrix,  trainable=True, name= 'Hmatrix')
        ...
        if self.Qnetwork == "Fmlp":
            ...
            #self.PrevWeightGRUQ = self.add_weight(shape=[  self._lsd**2 , 50], name="gruprevweight", initializer='random_normal')# (lsd**2, 50)
            
        if self.Qnetwork == "Xmlp":
            #build Q mlp parameters
            self._layer_Q_MLP = k.layers.Dense(self._lsd, activation=lambda x: k.activations.elu(x) + 1)
            #self.PrevWeightGRUQ = self.add_weight(shape=[  self._lsd , 50], name="gruprevweight", initializer='random_normal')# (lsd**2, 50)
            
        if self.Qnetwork == "Fgru":
            #build Q gru parameters
            self.GRUQunit = 2 * self._lsd * 4
