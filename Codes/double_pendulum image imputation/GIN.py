...
class GIN(k.models.Model):### GIN model includes encoder(or MLP), trnsition cell and decoder(or MLP)


    def __init__(self, obs_shape, obs_dim, out_dim, num_basis, 
                 trans_net_hidden_units=[], never_invalid=False, cell_type="gin", Qnetwork="Xmlp", USE_CONV = False, Smoothing = False):
        """
    ...
    def training(self, model, Train_Obs, Train_Obs_Valid, Train_Target, Valid_Obs, Valid_Obs_Valid, Valid_Target, epochs, batch_size):
        ...
        for bid in range(int(len(Valid_Obs_Valid)/batch_size)):
            Valid_ValidBatch.append( Valid_Obs_Valid[bid*batch_size:(bid+1)*batch_size])
        Valid_Ybatch = np.array(Valid_Ybatch)
        Valid_Ubatch = np.array(Valid_Ubatch)
        Valid_ValidBatch = np.array(Valid_ValidBatch)
        ...
        for bid in range(int(len(Train_Obs_Valid)/batch_size)):
            ValidBatch.append( Train_Obs_Valid[bid*batch_size:(bid+1)*batch_size])
        Ybatch = np.array(Ybatch)
        Ubatch = np.array(Ubatch)
        ValidBatch = np.array(ValidBatch)
        
        Training_Loss = []
        for epoch in range(epochs):
            for i in range(len(Ybatch)):
                NetIn = (Ubatch[i], ValidBatch[i])
                with tf.GradientTape() as tape:
                    preds = model(NetIn)
                    loss = self.bernoulli_nll(Ybatch[i], preds)
                ...
                if i %10==0:
                    ...
                    val_preds = model((Valid_Ubatch[rand_sel], Valid_ValidBatch[rand_sel]))
                    val_loss = self.bernoulli_nll(Valid_Ybatch[rand_sel], val_preds)
                    print('val loss: %s' % (val_loss.numpy()))
                
                print('epoch: %d  loss_Bernoulli: %s' % (epoch, loss.numpy()))
                
                Training_Loss.append(loss)  
        return Training_Loss
    
    def testing(self, model, test_obs, test_obs_valid, test_targets, batch_size):
        """
        testing procedure
        depending on the task, appropriate loss function is taken account
        """
        ...
        for bid in range(int(len(test_obs_valid)/batch_size)):
            ValidBatch.append( test_obs_valid[bid*batch_size:(bid+1)*batch_size])
        Ybatch = np.array(Ybatch)
        Ubatch = np.array(Ubatch)
        ValidBatch = np.array(ValidBatch)
