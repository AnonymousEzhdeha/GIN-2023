...
def main():
    ...
    batch_size_list = [10, 10, 10, 10, 10, 20, 25, 30, 40, 50, 100]
    MSE_TEST = []
    
    for length_train, batch_size in zip(train_samples, batch_size_list):
        ##data gen
        train_obs, train_targets, test_obs, test_targets, valid_obs, valid_targets = Generate_Data(num_seqs_train=1, num_seqs_test=1,
                               num_seqs_valid=1, seq_length_train=length_train, seq_length_test=5000, seq_length_valid=400, q=q, r=r)
        ...
        # # Train Model
        epochs, batch_size = 100, batch_size
        Training_Loss = Lorenz.training( Lorenz, sp_train_obs, sp_train_targets,
                                     sp_valid_obs, sp_valid_targets, epochs, batch_size)
        Test_Loss = Lorenz.testing( Lorenz, sp_test_obs, sp_test_targets, batch_size)
